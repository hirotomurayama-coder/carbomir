...
import { CaseStudyEditClient } from "@/components/admin/case-study-edit-client";
...
export default async function AdminEditCaseStudyPage({ params }: Props) {
  const { slug } = await params;
  const study = findBySlug<CaseStudy>("case-studies", slug);
  if (!study) notFound();
    ...
    <div className="px-6 sm:px-8 py-8 max-w-[1100px] mx-auto">
      ...
      <header className="mb-4">
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          ...
          <span className="font-mono text-[10.5px] text-muted-foreground">{study.slug}</span>
        </div>
      ...
      <AdminBanner />
      <CaseStudyEditClient initial={study} />
    </div>
  );
}
