...
const ENTITY_COPY: Record<EntityType, (name: string) => ConsultCopy> = {
  regulation: (name) => ({
    title: "貴社への影響評価・対応方針",
    body: `本ページでは ${name} の制度構造と論点を整理した。開示・調達・コストへの効き方と対応の優先順位づけは貴社固有の判断になる。クレイドルトゥーが対応する。`,
  }),
  methodology: (name) => ({
    title: "調達・創出計画への落とし込み",
    body: `本ページでは ${name} の手法・適格性を構造化した。貴社のポートフォリオへの組み込みと個別プロジェクトのデューデリは会社固有の判断になる。クレイドルトゥーが対応する。`,
  }),
  technology: (name) => ({
    title: "技術選定・事業化のご相談",
    body: `本ページでは ${name} の技術・市場を整理した。貴社の脱炭素計画や投資判断への落とし込みは個別案件の判断になる。クレイドルトゥーが対応する。`,
  }),
  market: (name) => ({
    title: "市場動向の個別評価",
    body: `本ページでは ${name} の構造と論点を整理した。貴社の調達・開示戦略への翻訳は会社固有の判断になる。クレイドルトゥーが対応する。`,
  }),
  player: (name) => ({
    title: "取引・選定のご相談",
    body: `本ページでは ${name} の役割を整理した。取引条件の評価・選定は個別案件の判断になる。クレイドルトゥーが対応する。`,
  }),
  project: (name) => ({
    title: "個別プロジェクトの評価",
    body: `本ページでは ${name} の概要を整理した。投資・調達の可否判断と現地デューデリは会社固有の判断になる。クレイドルトゥーが対応する。`,
  }),
};
...
export function consultCopyForTimeline(): ConsultCopy {
  return {
    title: "この動きの影響評価・対応",
    body: "本ページではイベントの解釈と関連を整理した。貴社の事業・開示・調達にどう響くかの評価と対応方針は会社固有の判断になる。クレイドルトゥーが対応する。",
  };
}
